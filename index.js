var keyButton = document.querySelectorAll(".drum");

for (var i = 0; i < keyButton.length; i++) {
  keyButton[i].addEventListener("click", function () {
    var buttonValue = this.innerHTML;
    toPlayOrNotToPlay(buttonValue);
    buttonAnimation(buttonValue);
  });
}

document.addEventListener("keydown", function(event){
  toPlayOrNotToPlay(event.key);
  buttonAnimation(event.key);
  console.log(event.key);
});

function toPlayOrNotToPlay(key) {
  switch (key) {
    case "w":
...
function buttonAnimation(currentKey) {
  ...
  setInterval(function() {activeButton.classList.remove("pressed")}, 200);
}
